...
test('loaded creators cover all channels', async () => {
  ...
  const creators = await loadCreators();
  const excluded = [ 'apple-talk', 'avoidclimatechange', 'nextlevelworldbuilding', 'dinnerplan', 'dex', 'edith', 'faithless', 'getaway', 'one-villainous-scene', 'one-x-cellent-scene', 'rng', 'scav', 'trussissues', 'theeditorial', 'layover', 'theprince', 'wtf', 'workingtitles', 'lostandfound', 'goonworld', 'drugs', 'showrunners', 'tataw', 'beyondthebleachers', 'extrahistorypresents' ];
